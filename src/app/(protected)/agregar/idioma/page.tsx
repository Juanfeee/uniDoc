...
const AgregarIdioma = () => {
  ...
  const [nivelesIdioma, setNivelesIdioma] = useState<{ value: string; label: string }[]>([]);

  // Cargar los niveles de idioma desde la API
  useEffect(() => {
    const fetchNivelesIdioma = async () => {
      try {
        const response = await axios.get(process.env.NEXT_PUBLIC_API_URL + "/constantes/niveles-idioma", {
          headers: {
        ...
        const niveles = response.data.nivel_idioma;
        const opcionesFormateadas = niveles.map((nivel: string) => ({
          value: nivel,
          label: nivel
        }));
        setNivelesIdioma(opcionesFormateadas);

      } catch (error) {
        console.error("Error al cargar los niveles de idioma", error);
      }
    };
